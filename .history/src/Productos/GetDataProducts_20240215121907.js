        ...
        <div className='container text-center mt-5'>
            ...
            {editProducto.id_producto && (
                <div>
                    ...
                    />
                    <select className='form-control mt-3' value={editProducto.categoria} onChange={(e) => setEditProducto({ ...editProducto, categoria: e.target.value })}>
                        <option value="">Selecciona una categoría</option>
                        {categorias.map(categoria => (
                            <option key={categoria.id_categoria} value={categoria.id_categoria}>{categoria.descripcion}</option>
                        ))}
                    </select>
                    <select className='form-control mt-3' value={editProducto.proveedor} onChange={(e) => setEditProducto({ ...editProducto, proveedor: e.target.value })}>
                        <option value="">Selecciona un proveedor</option>
                        {proveedores.map(proveedor => (
                            <option key={proveedor.id_proveedor} value={proveedor.id_proveedor}>{proveedor.nombre}</option>
                        ))}
